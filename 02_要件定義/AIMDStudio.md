...
| **フォルダ/ファイル追加・削除** | ルート目次（`00_目次.md` / `01_目次.md`）のリンクテーブルおよび他ファイル内の参照リンクを自動更新。<br>※英語環境時は、前書きなしの場合 `00_Contents.md`、前書きありの場合 `00_Introduction.md` + `01_Contents.md` で自動生成。 | ユーザーによる目次リンクの不正な手動変更を検知・ガード。 |
