...
| :--- | :--- | :--- | :--- |
| **AIMDモード** | ルートフォルダ名が `.aimd` | 構造化ドキュメント編集に特化した完全支援モード | ・AIMD Explorer（専用ツリー）<br>・階層別専用アイコン表示<br>・メタデータ（`.aimd-meta/`）保護<br>・ルート目次（`index.md`）自動同期・保護<br>・章ごとの画像自動保存・共通化解決<br>・構成バリデーション（Quick Fix）<br>・ハーネスAI連携（Preview Diff） |
...
| **ファイル命名規則** | ・入力あり：`XX-ファイル名_figYY_(入力名称).png`<br>・未入力/スキップ：`XX-ファイル名_figYY.png`（連番のみ） | 同左 |
| **保存先フォルダ** | 紐づく親の章の **`99-Image/`**（本文内に小見出しがあっても親の章に集約） | 編集中のMarkdownファイルと**同一階層の `asset/`** |
| **重複・共通化対応** | 複数章で同名画像が使われた場合、確認ダイアログを経て親階層の `99-Image/` に昇格し、全参照パスを一括更新 | なし（同一階層に連番等で保存） |
| **不要画像クリーンアップ** | 全Markdownを走査し、本文から参照されていない `99-Image/` 配下の画像を洗い出して一括削除する専用コマンドを提供 | 対象外 |
...
| **フォルダ/ファイル追加・削除** | ルート目次（`index.md` / `目次.md`）のリンクテーブルおよび他ファイル内の参照リンクを自動更新。<br>※英語環境時は `index.md` および `00-Introduction.md`（初めに）で自動生成。 | ユーザーによる目次リンクの不正な手動変更を検知・ガード。 |
| **ツリー上の並び替え（D&D）** | 物理フォルダ名の連番プレフィックス（`02-` 等）と目次内リンクを自動同期してリネーム。 | 「初めに」の追加・削除による大規模な連番変更時は、警告ダイアログで承認を得てから実行。 |
| **構成の崩れ検知（バリデーション）** | 連番重複、拡張子不正、`99-Image` 以外の画像フォルダ等を検知し、右下に警告トーストを表示。 | 問題一覧の提示とともに、ワンクリック自動修正（Quick Fix）を提供。 |
...
sequenceDiagram
    ...
    Ext->>Ext: ファイル名確定 (XX-ファイル名_figYY_名称.png / 連番)
    ...
    alt 通常モードの場合
        ...
        alt 複数章で既に同一名称の画像が存在
            ...
            ユーザー-->>Ext: 承認
            Ext->>FS: 親階層またはルートの「99-Image/」に保存
            Ext->>FS: 既存Markdown内の参照パスを一括更新
            Ext->>Editor: 共通画像パスリンクを挿入 (../99-Image/...)
        else 通常の章内固有画像
            Ext->>FS: 紐づく親の章の「99-Image/」に保存
            Ext->>Editor: 相対パスリンクを挿入 (./99-Image/...)
        end
    end
```
...
flowchart TD
    ...
    CheckPrefix --> ReadIndex[ルート目次 index.md / 目次.md を読み込み]
    
    ReadIndex --> IntroCheck{「00-初めに.md」が<br/>存在するか？}
